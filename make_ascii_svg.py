...
CURSOR = "#c9d1d9"
ACCENT = "#00ff00"   # green accent glow
...
parts.append('<defs>'
             ...
             f'<filter id="greenGlow"><feGaussianBlur stdDeviation="3" result="coloredBlur"/>'
             f'<feFlood flood-color="{ACCENT}" flood-opacity="0.4" result="coloredBlur" in="coloredBlur"/>'
             f'<feMerge><feMergeNode in="coloredBlur"/><feMergeNode in="SourceGraphic"/></feMerge></filter>'
             f'</defs>')

parts.append(f'<rect width="{CANVAS_W}" height="{CANVAS_H}" rx="12" fill="url(#bg)"/>')
parts.append(f'<rect x="0.5" y="0.5" width="{CANVAS_W-1}" height="{CANVAS_H-1}" rx="12" '
             f'fill="none" stroke="{ACCENT}" stroke-width="2" opacity="0.6"/>')
parts.append(f'<rect x="0.5" y="0.5" width="{CANVAS_W-1}" height="{CANVAS_H-1}" rx="12" '
             f'fill="none" stroke="{FRAME}" stroke-width="1"/>')

parts.append(f'<line x1="0" y1="{TITLEBAR_H}" x2="{CANVAS_W}" y2="{TITLEBAR_H}" stroke="{ACCENT}" stroke-width="2" opacity="0.4"/>')
parts.append(f'<line x1="0" y1="{TITLEBAR_H}" x2="{CANVAS_W}" y2="{TITLEBAR_H}" stroke="{FRAME}"/>')
for i, dotcol in enumerate([ACCENT if i == 2 else ("#ff5f56" if i == 0 else "#ffbd2e") for i in range(3)]):
    opacity = "0.8" if i == 2 else "1"
    parts.append(f'<circle cx="{PAD + i*16}" cy="{TITLEBAR_H/2}" r="5" fill="{dotcol}" opacity="{opacity}"/>')
...
for ry, line in enumerate(rows_txt):
    ...
    text = (f'<text xml:space="preserve" x="{PAD}" y="{y:.1f}" fill="{INK}" '
            f'font-size="{font_size:.1f}" textLength="{ART_W}" lengthAdjust="spacing" filter="url(#greenGlow)">{safe}</text>')
